...
func run() error {
	...
	schedulerSvc.AddJob(parquetJob)
	...
	if err := jobConfigStore.Seed(bootstrapCtx, []jobconfig.JobConfig{
		...
		{JobName: parquetJob.Name(), CronExpr: cfg.ParquetCron, Enabled: true},
	}); err != nil {
	...
	allJobs := []worker.Job{benchmarkSyncJob, scenarioStatsJob, leaderboardJob, parquetJob}
